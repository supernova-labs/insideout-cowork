#!/usr/bin/env python3
"""Validação estrutural determinística das skills locais da transformação."""
...
def main() -> int:
    ...
    agents_root = ROOT.parent.parent / ".agents" / "skills"
    for name, skill_dir in SKILLS.items():
        adapter = agents_root / name
        if not adapter.is_symlink():
            warnings.append(f"adapter de descoberta ausente: .agents/skills/{name}")
            continue
        if adapter.resolve() != skill_dir.resolve():
            errors.append(f"adapter de {name} aponta para destino incorreto")
